...
import com.owner.es.model.Book;
import com.owner.es.model.Msg;
import com.owner.es.repository.BookRepository;
 ...
 *
 * restfulAPI的优点
 * 不用费力的拼 getBook?id=1&name=2  使用PathVariable简单了很多 /book/1001  1001直接作为请求参数  get请求就是查询
 *
...
public class BookController {
     ...
     *
     * localhost:8888/book   参数在body里的json
     *
     * 新增 如果不传id,elasticsearch会自动创建id
     */
    // 演示：入参使用json格式的对象（使用@RequestBody注解，paramType可省略，name名与对象名保持一致）
    @ApiOperation(value = "新增一本书")
    @ApiImplicitParam(name = "book",value = "book的实体信息",required = true,dataType = "Book",paramType = "body")
    @PostMapping(name = "/")
    public Msg postBook(@RequestBody Book book) {
     ...
     *
     * localhost:8888/book/{id}  delete请求方式, 如果新增的时候,没有指定id,那么删除时, 就指定用kibana的_id来删除
     *
    ...
    @ApiImplicitParam(name = "id",value = "book的主键id",required = true,dataType = "String",paramType = "path")
    @DeleteMapping("/{id}")
    ...
    /**
     * 修改
     * localhost:8888/book/{id}  put请求  body中的json,id必须给传上,不传的话就是新增了
     *
     * @param book
     * @return
     */
    ...
    @ApiImplicitParam(name = "book",value = "book的实体信息",required = true,dataType = "Book",paramType = "body")
    @PutMapping("/{id}")
    public Msg putBook(@PathVariable("id") String id, @RequestBody Book book) {
    ...
    /**
     * searchContent
     * localhost:8888/book/getListByPage/1/3/余华
     *
     * @param page
     * @param size
     * @param q
...
//    swagger 中@ApiImplicitParam()跟@ApiParam()两个注解的区别
    ...
    @ApiImplicitParams({
        ...
        @ApiImplicitParam(name = "size",value = "book表的主键id",required = true, dataType = "String",paramType = "path"),
        @ApiImplicitParam(name = "q",value = "全文检索的关键字",required = false, dataType = "String",paramType = "path")
    })
    @GetMapping("/getListByPage/{page}/{size}/{q}")
    public Page<Book> getListByPage(@PathVariable Integer page, @PathVariable Integer size, @PathVariable String q) {
...
//        PageRequest pageable = PageRequest.of(page - 1, size, sort);
        ...
        QueryStringQueryBuilder builder = new QueryStringQueryBuilder(q);
        Page<Book> bookList = bookRepository.search(builder, pageable);//全文检索 所有分词的字段

//        Spring Data翻页查询总是返回Page对象，Page对象提供了以下常用的方法：
        int totalPages = bookList.getTotalPages();//总页数
        List<Book> content = bookList.getContent();//所有的book对象
        long totalElements = bookList.getTotalElements();//返回总记录数目
        int pageNumber = bookList.getPageable().getPageNumber();// 0
        int pageSize = bookList.getPageable().getPageSize();// 10
        return bookList;
//        Sort sort1 = new Sort("name");//默认升序
//        Page<Book> all = bookRepository.findAll(pageable);//返回所有实体，按照指定顺序排序返回
//        Page<Book> bookList = bookRepository.findByName("测试", PageRequest.of(0, 2, sort));
